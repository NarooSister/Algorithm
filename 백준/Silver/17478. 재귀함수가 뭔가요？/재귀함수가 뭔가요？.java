import java.io.BufferedReader;
import java.io.IOException;
import java.io.InputStreamReader;

public class Main {
    static String start = "어느 한 컴퓨터공학과 학생이 유명한 교수님을 찾아가 물었다.";
    static String question = "\"재귀함수가 뭔가요?\"";
    static String answer1 = "\"잘 들어보게. 옛날옛날 한 산 꼭대기에 이세상 모든 지식을 통달한 선인이 있었어.\n";
    static String answer2 = "마을 사람들은 모두 그 선인에게 수많은 질문을 했고, 모두 지혜롭게 대답해 주었지.\n";
    static String answer3 = "그의 답은 대부분 옳았다고 하네. 그런데 어느 날, 그 선인에게 한 선비가 찾아와서 물었어.\"";
    static String lastAnswer = "\"재귀함수는 자기 자신을 호출하는 함수라네\"";
    static String end = "라고 답변하였지.";
    static String str = "";
    static StringBuilder sb = new StringBuilder();

    public static void main(String[] args) throws IOException {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        sb.append(start);
        func(Integer.parseInt(br.readLine()), 0);

        System.out.println(sb.toString());
    }

    public static void func(int n, int depth) {

        if (depth == n) {
            sb.append("\n").append(str).append(question).append("\n").append(str).append(lastAnswer).append("\n").append(str).append(end);
            return;
        }
        sb.append("\n").append(str).append(question).append("\n").append(str).append(answer1).append(str).append(answer2).append(str).append(answer3);
        str += "____";
        depth++;
        func(n, depth);

        str = str.substring(0, str.length() - 4);
        sb.append("\n").append(str).append(end);
    }
}